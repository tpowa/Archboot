...
_set_vconsole() {
    ...
    #shellcheck disable=SC2086
    _dialog --title "${_TITLE}" --menu "Select A Keymap Region:" 13 40 7 ${_KEYMAPS} 2>${_ANSWER} || return 1
    _KEYMAP=$(cat ${_ANSWER})
    if [[ "${_KEYMAP}" == "OTHER" ]]; then
        #shellcheck disable=SC2086
        _dialog --title "${_TITLE}" --menu "Select A Keymap Region:" 18 40 12 ${_OTHER_KEYMAPS} 2>${_ANSWER} || return 1
        _KEYMAP=$(cat ${_ANSWER})
    fi
    _KEYMAPS=""
    ...
    #shellcheck disable=SC2086
    _dialog --title "${_TITLE}" --menu "Select A Keymap Layout:" 14 40 8 ${_KEYMAPS} 2>${_ANSWER} || return 1
    #shellcheck disable=SC2086
    _KEYMAP=$(cat ${_ANSWER})
}
