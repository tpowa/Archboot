...
_localize() {
    ...
    echo | _dialog --no-mouse --gauge "Setting locale to ${_LOCALE}.UTF-8..." 3 50 0
    locale-gen &>/dev/null
    sleep 1
    echo | _dialog --no-mouse --gauge "Setting vconsole font to ${_FONT}..." 3 50 33
    sleep 1
    echo | _dialog --no-mouse --gauge "Setting keymap to ${_KEYMAP}..." 3 50 66
    sleep 1
    echo | _dialog --no-mouse --gauge "Localization completed successfully." 3 50 100
    sleep 1
}
