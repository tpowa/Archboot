...
_wireless() {
    ...
    if [[ -z "${_WLAN_KEY}" ]]; then
        ...
        echo "iwctl --passphrase=\"${_WLAN_KEY}\" station \"${_INTERFACE}\" \"${_WLAN_CONNECT}\" \"${_WLAN_SSID}\" &>\"${_NO_LOG}\"" >> ${_TEMPLATE}
    fi
    echo "### wireless authentification end" >> ${_TEMPLATE}
}
...
_network() {
    ...
    # write to template
    echo "### network profile start" >> ${_TEMPLATE}
    echo "echo Network..." >> ${_TEMPLATE}
    echo "echo \"#$_NETWORK_PROFILE generated by Archboot setup\" > \"${_NETWORK_PROFILE}\"" >> ${_TEMPLATE}
    #shellcheck disable=SC2129
    echo "echo \"[Match]\"  >> \"${_NETWORK_PROFILE}\"" >> ${_TEMPLATE}
    echo "echo \"Name=${_INTERFACE}\" >> \"${_NETWORK_PROFILE}\"" >> ${_TEMPLATE}
    echo "echo \"\" >> \"${_NETWORK_PROFILE}\"" >> ${_TEMPLATE}
    echo "echo \"[Network]\" >> \"${_NETWORK_PROFILE}\"" >> ${_TEMPLATE}
    echo "echo \"MulticastDNS=yes\" >> \"${_NETWORK_PROFILE}\"" >> ${_TEMPLATE}
    [[ "${_IP}" == "dhcp" ]] && echo "echo \"DHCP=yes\" >> \"${_NETWORK_PROFILE}\"" >> ${_TEMPLATE}
    if [[ "${_CONNECTION}" == "wireless" ]]; then
        #shellcheck disable=SC2129
        echo "echo \"IgnoreCarrierLoss=3s\" >>\"${_NETWORK_PROFILE}\"" >> ${_TEMPLATE}
    fi
    if [[ "${_IP}" == "static" ]]; then
        #shellcheck disable=SC2129
        echo "echo \"Address=${_IPADDR}\" >>\"${_NETWORK_PROFILE}\"" >> ${_TEMPLATE}
        echo "echo \"Gateway=${_GW}\" >>\"${_NETWORK_PROFILE}\"" >> ${_TEMPLATE}
        echo "echo \"DNS=${_DNS}\" >>\"${_NETWORK_PROFILE}\"" >> ${_TEMPLATE}
    fi
    # set proxies
    if [[ -z "${_PROXY}" ]]; then
        for i in ${_PROXIES}; do
            echo "unset \"${i}\""  >> ${_TEMPLATE}
        done
    else
        for i in ${_PROXIES}; do
            echo "export \"${i}\"=\"${_PROXY}\"" >> ${_TEMPLATE}
            echo "echo \"export ${i}=${_PROXY}\" >> /etc/profile.d/proxy.sh" >> ${_TEMPLATE}
