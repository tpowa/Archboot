...
_wireless() {
    ...
    if [[ -n "${_WLAN_AUTH}" ]]; then
        ...
        { echo "### wireless authentification start"
...
_network() {
    ...
    # write to template
    { echo "### network profile start"
    echo "echo Network..."
    echo "echo \"#$_NETWORK_PROFILE generated by Archboot setup\" > \"${_NETWORK_PROFILE}\""
    #shellcheck disable=SC2129
    echo "echo \"[Match]\"  >> \"${_NETWORK_PROFILE}\""
    echo "echo \"Name=${_INTERFACE}\" >> \"${_NETWORK_PROFILE}\""
    echo "echo \"\" >> \"${_NETWORK_PROFILE}\""
    echo "echo \"[Network]\" >> \"${_NETWORK_PROFILE}\""
    echo "echo \"MulticastDNS=yes\" >> \"${_NETWORK_PROFILE}\""
    [[ "${_IP}" == "dhcp" ]] && echo "echo \"DHCP=yes\" >> \"${_NETWORK_PROFILE}\""
    if [[ "${_CONNECTION}" == "wireless" ]]; then
        #shellcheck disable=SC2129
        echo "echo \"IgnoreCarrierLoss=3s\" >>\"${_NETWORK_PROFILE}\""
    fi
    if [[ "${_IP}" == "static" ]]; then
        #shellcheck disable=SC2129
        echo "echo \"Address=${_IPADDR}\" >>\"${_NETWORK_PROFILE}\""
        echo "echo \"Gateway=${_GW}\" >>\"${_NETWORK_PROFILE}\""
        echo "echo \"DNS=${_DNS}\" >>\"${_NETWORK_PROFILE}\""
    fi
    # set proxies
    if [[ -z "${_PROXY}" ]]; then
        for i in ${_PROXIES}; do
    ...
    echo "echo Waiting 10 seconds for network link..."
    echo "sleep 10"
    echo ": > /.network"
