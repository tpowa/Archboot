...
_select_mirror() {
    ...
    if [[ "${_RUNNING_ARCH}" == "x86_64" ]]; then
        ...
        ${_DLPROG} "https://www.archlinux.org/mirrorlist/?country=$(curl http://ip-api.com/csv/?fields=countryCode)&protocol=http&protocol=https&ip_version=4&ip_version=6&use_mirror_status=on" -O /tmp/pacman_mirrorlist.txt
