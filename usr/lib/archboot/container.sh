...
_clean_container() {
    if [[ "${_CLEANUP_CONTAINER}" ==  "1" ]]; then
        ...
        rm -r "${1}"/usr/lib/{audit,awk,binfmt.d,cmake,dracut,e2fsprogs,environment.d,gawk,getconf,gettext,glib-[0-9]*,gssproxy,icu,krb5,ldscripts,lib{nl,gallium*,go.*,gphobos*,LLVM*},pkgconfig,python*,siconv,tar,xfsprogs}
        # locale cleaning
