...
_install_gnome_wayland() {
    if ! [[ -e /usr/bin/gnome-session ]]; then
        echo -e "\033[1mStep 1/3:\033[0m Installing GNOME Wayland now ..."
        echo "          This will need some time ..."
        _prepare_graphic "${_WAYLAND_PACKAGE} ${_STANDARD_PACKAGES} ${_GNOME_PACKAGES}" >/dev/tty7 2>&1
        echo -e "\033[1mStep 2/3:\033[0m Configuring GNOME Wayland ..."
        _configure_gnome_wayland >/dev/tty7 2>&1
        systemd-sysusers >/dev/tty7 2>&1
        systemd-tmpfiles --create >/dev/tty7 2>&1
    else
        echo -e "\033[1mStep 1/3:\033[0m Installing GNOME Wayland already done ..."
        echo -e "\033[1mStep 2/3:\033[0m Configuring GNOME Wayland already done ..."
    fi
}
