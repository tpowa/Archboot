...
find_btrfs_subvolume() {
    if [[ "${DETECT_CREATE_FILESYSTEM}" = "no" ]]; then
        ...
        mount_btrfs
        for i in $(btrfs subvolume list "${BTRFSMP}" | cut -d " " -f 9 | grep -v 'var/lib/machines' | grep -v '/var/lib/portables'); do
            echo "${i}"
            [[ "${1}" ]] && echo "${1}"
        done
        umount_btrfs
...
btrfs_raid_level() {
    ...
    while [[ "${BTRFS_RAID_FINISH}" != "DONE" ]]; do
        ...
        DIALOG --menu "Select the raid data level you want to use" 12 50 9 ${BTRFS_RAIDLEVELS} 2>"${ANSWER}" || return 1
