...
detect_DISC() {
    if [[ "${_DISC}" == "" ]] || ! echo "${_DISC}" | grep -q '/dev/'; then
        _DISC="$(${_LSBLK} PKNAME "$(findmnt -vno SOURCE "${_DESTDIR}/boot")")"
    fi
    if [[ "${_DISC}" == "" ]]; then
        _DISC="$(${_LSBLK} PKNAME "$(findmnt -vno SOURCE "${_DESTDIR}/")")"
    fi
}
...
check_gpt() {
    ...
    if [[ -z "${_GUID_DETECTED}" ]]; then
        ...
        if [[ -z "${_GUID_DETECTED}" ]]; then
            ...
            dd if=/dev/zero of="${_DISC}" bs=512 count=2048 &>/dev/null
            wipefs -a "${_DISC}" &>/dev/null
            # create fresh GPT
            sgdisk --clear "${_DISC}" &>/dev/null
            _GUID_DETECTED="1"
        fi
    fi
