...
_autoprepare() {
    ...
    while [[ -z "${_DEFAULTFS}" ]]; do
        ...
        while [[ -z "${_SWAPDEV_SET}" ]]; do
            _dialog --title " Swap In MiB " --no-cancel --inputbox "Value 0 will skip Swap creation\nDisk space left: ${_DISK_SIZE}M |" 9 70 "${_SWAP_SIZE}" 2>"${_ANSWER}" || return 1
        ...
        _dialog --title " / in MiB " --inputbox "Value 0 will skip /home creation and use the rest ${_DISK_SIZE}M for /\nDisk space left: $((_DISK_SIZE-350))M | Minimum value is 2000" 9 70 "${_ROOT_SIZE}" 2>"${_ANSWER}" || return 1
