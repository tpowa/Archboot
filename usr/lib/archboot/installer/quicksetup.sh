...
_autoprepare() {
    ...
    while [[ -z "${_DEFAULTFS}" ]]; do
        ...
        while [[ -z "${_SWAPDEV_SET}" ]]; do
            _dialog --title " Swap In MiB " --no-cancel --inputbox "Disk space left: ${_DISK_SIZE}M | Value 0 will skip Swap partition creation" 8 70 "${_SWAP_SIZE}" 2>"${_ANSWER}" || return 1
        ...
        _dialog --title " / in MiB " --inputbox "Disk space left: $((_DISK_SIZE-350))M | Minimum value is 2000\nValue 0 will skip /home partition creation and use rest ${_DISK_SIZE}M for /" 9 70 "${_ROOT_SIZE}" 2>"${_ANSWER}" || return 1
