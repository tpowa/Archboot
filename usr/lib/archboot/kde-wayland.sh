...
_install_kde_wayland() {
    if ! [[ -e /usr/bin/startplasma-x11 ]]; then
        ...
        _prepare_graphic "plasma-wayland-session egl-wayland ${_KDE_PACKAGES}" >/dev/tty7 2>&1
...
_start_kde_wayland() {
    echo -e "Launching KDE now, logging is done on \033[1m/dev/tty7\033[0m ..."
    exec dbus-run-session startplasma-wayland >/dev/tty7 2>&1
    echo -e "To relaunch KDE desktop wayland use: \033[92mupdate-installer.sh -kde-wayland\033[0m"
}
