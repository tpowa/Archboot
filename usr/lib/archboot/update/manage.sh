...
_geoip_mirrorlist() {
    if [[ "${_RUNNING_ARCH}" == "x86_64" && ! -e /.pacsetup  ]]; then
        _COUNTRY="$(${_DLPROG} --max-time 10 "http://ip-api.com/csv/?fields=countryCode")"
        echo "GeoIP country ${_COUNTRY} detected." >>"${_LOG}"
        ${_DLPROG} --max-time 10 -o /tmp/pacman_mirrorlist.txt "https://www.archlinux.org/mirrorlist/?country=${_COUNTRY}&protocol=https&ip_version=4&ip_version=6&use_mirror_status=on"
...
_clean_fw() {
    ...
    if rg -q "${_WIFI}" "${_PCI}"; then
        ...
        if rg "${_WIFI}" "${_PCI}" | rg -q 'Mediatek'; then
            mv "${_FW}"/{mt76*,mediatek,amphion} ${_FW_NEW}/
        fi
...
_download_latest_task() {
    ...
    _SCRIPTS="binary-check.sh fw-check.sh not-installed.sh package-check.sh secureboot-keys.sh mkkeys.sh hwsim.sh"
    for i in ${_SCRIPTS}; do
...
_new_image() {
    ...
    if ! [[ "${_RUNNING_ARCH}" == "riscv64" ]]; then
        ...
        : > "${_W_DIR}"/.archboot
        (${_NSPAWN} "${_W_DIR}" /bin/bash -c "umount /tmp;rm -rf /tmp/*; archboot-${_RUNNING_ARCH}-iso.sh -g -s -c=${_CONFIG_LOCAL} \
        ...
        : > "${_W_DIR}"/.archboot
        (${_NSPAWN} "${_W_DIR}" /bin/bash -c "umount /tmp;rm -rf /tmp/*;archboot-${_RUNNING_ARCH}-iso.sh -g -s -c=${_CONFIG_LATEST} \
